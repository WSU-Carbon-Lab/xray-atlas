...
exports.isTtlModified =
  exports.toCreateTableInput =
  exports.extractOldTableInputFromEvent =
  exports.extractTableInputFromEvent =
  exports.getPointInTimeRecoveryUpdate =
  exports.getTtlUpdate =
  exports.getDeletionProtectionUpdate =
  exports.requiresTagsUpdate =
  exports.getSseUpdate =
  exports.getStreamUpdate =
  exports.getNextAtomicUpdate =
  exports.processIsComplete =
  exports.getLambdaTags =
  exports.isComplete =
  exports.onEvent =
    void 0;
...
const import_table_1 = require("./import-table");
...
const getLambdaTags = async (functionArn) => {
  ...
  const tags =
    (_a = (await lambdaClient.send(command)).Tags) !== null && _a !== void 0
      ? _a
      : {};
  const result = [];
...
const getTableTags = async (tableArn) => {
  ...
  const tags =
    (_a = (await ddbClient.send(command)).Tags) !== null && _a !== void 0
      ? _a
      : [];
  return tags;
};
async function onEventHandler(cfnRequest, context) {
  ...
  if (cfnRequest.RequestType == "Delete") {
    ...
    await cfnResponse.submitResponse("SUCCESS", resourceEvent, {
...
async function isCompleteHandler(event, context) {
  ...
  const isCompleteResult = await (0, exports.processIsComplete)(
    sanitizedRequest,
    context,
  );
  ...
  if (!isCompleteResult.IsComplete) {
    if (
      ...
      Object.keys(isCompleteResult.Data).length > 0
    ) {
...
const processOnEvent = async (event, context) => {
  ...
  const tableDef = await (0, exports.extractTableInputFromEvent)(
    event,
    context,
  );
  ...
  switch (event.RequestType) {
    case "Create":
      ...
      if (tableDef.isImported) {
        return (0, import_table_1.importTable)(createTableInput);
      }
    ...
    case "Update":
      if (!event.PhysicalResourceId) {
        throw new Error(
          `Could not find the physical ID for the updated resource`,
        );
      }
      const oldTableDef = (0, exports.extractOldTableInputFromEvent)(event);
      console.log("Fetching current table state");
      const describeTableResult = await ddbClient.describeTable({
        TableName: event.PhysicalResourceId,
      });
      ...
      if (
        isKeySchemaModified(
          describeTableResult.Table.KeySchema,
          tableDef.keySchema,
        )
      ) {
        console.log(
          "Detected table key schema change. Update requires replacement",
        );
      ...
      if (
        tableDef.replaceTableUponGsiUpdate &&
        tableDef.allowDestructiveGraphqlSchemaUpdates
      ) {
        const nextUpdate = getNextGSIUpdate(
      ...
      const currentTableTags = await getTableTags(
        describeTableResult.Table.TableArn,
      );
      const newTags = [];
      Object.values(
        (_a = tableDef.tags) !== null && _a !== void 0 ? _a : [],
      ).forEach((tag) => {
      ...
      if (removeTags.length > 0) {
        ...
        console.log(
          `Table '${event.PhysicalResourceId}' is ready after the deletion of Tags.`,
        );
      }
      if ((0, exports.requiresTagsUpdate)(currentTableTags, newTags)) {
        ...
        console.log(
          `Table '${event.PhysicalResourceId}' is ready after the update of Tags.`,
        );
      }
      const describePointInTimeRecoveryResult =
        await ddbClient.describeContinuousBackups({
          TableName: event.PhysicalResourceId,
        });
      console.log(
        "Current point in time recovery: ",
        describePointInTimeRecoveryResult,
      );
      const pointInTimeUpdate = (0, exports.getPointInTimeRecoveryUpdate)(
        describePointInTimeRecoveryResult.ContinuousBackupsDescription,
        tableDef,
      );
      if (pointInTimeUpdate) {
        (0, util_1.log)(
          "Computed point in time recovery update",
          pointInTimeUpdate,
        );
        ...
        console.log(
          `Table '${event.PhysicalResourceId}' is ready after the update of PointInTimeRecovery.`,
        );
      }
      const deletionProtectionUpdate = (0, exports.getDeletionProtectionUpdate)(
        describeTableResult.Table,
        tableDef,
      );
      if (deletionProtectionUpdate) {
        (0, util_1.log)(
          "Computed deletion protection update",
        ...
        console.log(
          `Table '${event.PhysicalResourceId}' is ready after the update of deletion protection.`,
        );
      }
      const sseUpdate = (0, exports.getSseUpdate)(
        describeTableResult.Table,
        tableDef,
      );
      if (sseUpdate) {
        ...
        console.log(
          `Table '${event.PhysicalResourceId}' is ready after the update of sever side encryption.`,
        );
      }
      const streamUpdate = await (0, exports.getStreamUpdate)(
        describeTableResult.Table,
        tableDef,
      );
      if (streamUpdate) {
        ...
        console.log(
          `Table '${event.PhysicalResourceId}' is ready after the update of stream specificaion.`,
        );
      }
      if (
        (0, exports.isTtlModified)(
          oldTableDef.timeToLiveSpecification,
          tableDef.timeToLiveSpecification,
        )
      ) {
        const describeTimeToLiveResult = await getTtlStatus(
          event.PhysicalResourceId,
        );
        console.log("Current TTL: ", describeTimeToLiveResult);
        const ttlUpdate = (0, exports.getTtlUpdate)(
          describeTimeToLiveResult.TimeToLiveDescription,
          tableDef,
        );
        if (ttlUpdate) {
      ...
      const nextGsiUpdate = (0, exports.getNextAtomicUpdate)(
        describeTableResult.Table,
        tableDef,
      );
      if (nextGsiUpdate) {
    ...
    case "Delete":
      ...
      try {
        console.log("Fetching current table state");
        const describeTableResultBeforeDeletion = await ddbClient.describeTable(
          { TableName: event.PhysicalResourceId },
        );
        if (
          (_b = describeTableResultBeforeDeletion.Table) === null ||
          _b === void 0
            ? void 0
            : _b.DeletionProtectionEnabled
        ) {
          return result;
        }
...
const processIsComplete = async (event, context) => {
  ...
  const describeTableResult = await retry(
    async () =>
      await ddbClient.describeTable({ TableName: event.PhysicalResourceId }),
    (result) =>
      !!(result === null || result === void 0 ? void 0 : result.Table),
  );
  if (
    ((_a = describeTableResult.Table) === null || _a === void 0
      ? void 0
      : _a.TableStatus) !== "ACTIVE"
  ) {
    console.log("Table not active yet");
    return notFinished;
  }
  if (
    (_b = describeTableResult.Table.GlobalSecondaryIndexes) === null ||
    _b === void 0
      ? void 0
      : _b.some((gsi) => gsi.IndexStatus !== "ACTIVE" || gsi.Backfilling)
  ) {
    console.log("Some GSI is not active yet");
    return notFinished;
  }
  const endState = await (0, exports.extractTableInputFromEvent)(
    event,
    context,
  );
  if (
    event.RequestType === "Create" ||
    ((_c = event.Data) === null || _c === void 0
      ? void 0
      : _c.IsTableReplaced) === true
  ) {
    const describePointInTimeRecoveryResult =
      await ddbClient.describeContinuousBackups({
        TableName: event.PhysicalResourceId,
    ...
    if (pointInTimeUpdate) {
      ...
      try {
        ...
        if (
          error instanceof
          client_dynamodb_1.ContinuousBackupsUnavailableException
        ) {
    ...
    if (
      endState.timeToLiveSpecification &&
      endState.timeToLiveSpecification.enabled
    ) {
      const describeTimeToLiveResult = await getTtlStatus(
        event.PhysicalResourceId,
      );
  ...
  const nextUpdate = (0, exports.getNextAtomicUpdate)(
    describeTableResult.Table,
    endState,
  );
...
exports.processIsComplete = processIsComplete;
const replaceTable = async (currentState, endState) => {
  if (currentState.DeletionProtectionEnabled === true) {
    throw new Error(
      "Table cannot be replaced when the deletion protection is enabled.",
    );
  }
  ...
  console.log(
    `Table '${currentState.TableName}' does not exist. Deletion is finished.`,
  );
...
const getNextAtomicUpdate = (currentState, endState) => {
  ...
  const isTableBillingModeModified =
    (((_a = currentState.BillingModeSummary) === null || _a === void 0
      ? void 0
      : _a.BillingMode) !== undefined &&
      ((_b = currentState.BillingModeSummary) === null || _b === void 0
        ? void 0
        : _b.BillingMode) !== endState.billingMode) ||
    (((_c = currentState.BillingModeSummary) === null || _c === void 0
      ? void 0
      : _c.BillingMode) == undefined &&
      endState.billingMode === "PAY_PER_REQUEST");
  const isTableProvisionThroughputModified =
    (((_d = endState.provisionedThroughput) === null || _d === void 0
      ? void 0
      : _d.readCapacityUnits) !== undefined &&
      ((_e = currentState.ProvisionedThroughput) === null || _e === void 0
        ? void 0
        : _e.ReadCapacityUnits) !==
        ((_f = endState.provisionedThroughput) === null || _f === void 0
          ? void 0
          : _f.readCapacityUnits)) ||
    (((_g = endState.provisionedThroughput) === null || _g === void 0
      ? void 0
      : _g.writeCapacityUnits) !== undefined &&
      ((_h = currentState.ProvisionedThroughput) === null || _h === void 0
        ? void 0
        : _h.WriteCapacityUnits) !==
        ((_j = endState.provisionedThroughput) === null || _j === void 0
          ? void 0
          : _j.writeCapacityUnits));
  if (isTableBillingModeModified || isTableProvisionThroughputModified) {
    let updateInput = {
      TableName: currentState.TableName,
      BillingMode: isTableBillingModeModified
        ? endState.billingMode
        : undefined,
      ProvisionedThroughput:
        isTableProvisionThroughputModified &&
        endState.billingMode === "PROVISIONED"
          ? {
              ReadCapacityUnits:
                (_k = endState.provisionedThroughput) === null || _k === void 0
                  ? void 0
                  : _k.readCapacityUnits,
              WriteCapacityUnits:
                (_l = endState.provisionedThroughput) === null || _l === void 0
                  ? void 0
                  : _l.writeCapacityUnits,
            }
          : undefined,
    };
    if (isTableBillingModeModified && endState.billingMode === "PROVISIONED") {
      const indexToBeUpdated = currentStateGSIs.map((gsiToUpdate) => {
        ...
        return {
          Update: {
            ...
            ProvisionedThroughput: {
              ReadCapacityUnits:
                (_a = endState.provisionedThroughput) === null || _a === void 0
                  ? void 0
                  : _a.readCapacityUnits,
              WriteCapacityUnits:
                (_b = endState.provisionedThroughput) === null || _b === void 0
                  ? void 0
                  : _b.writeCapacityUnits,
            },
          },
        };
      });
      updateInput = {
        ...
        GlobalSecondaryIndexUpdates:
          indexToBeUpdated.length > 0 ? indexToBeUpdated : undefined,
      };
    }
...
const getNextGSIUpdate = (currentState, endState) => {
  ...
  const modifiedAttributes = getTypeModifiedAttributes(
    currentState.AttributeDefinitions,
    endState.attributeDefinitions,
  );
  const indexesWithModifiedAttributeType = getIndexesContainingAttributes(
    currentState.GlobalSecondaryIndexes,
    modifiedAttributes,
  );
  const currentStateGSIs = currentState.GlobalSecondaryIndexes || [];
  ...
  const gsiRequiresReplacementPredicate = (currentGSI) => {
    ...
    if (
      ...
      return true;
    if (
      isKeySchemaModified(currentGSI.KeySchema, respectiveEndStateGSI.keySchema)
    )
      return true;
    return false;
  };
  ...
  const gsiRequiresCreationPredicate = (endStateGSI) =>
  ...
  if (gsiToAdd) {
    ...
    if (
      endState.billingMode === "PROVISIONED" &&
      gsiToAdd.provisionedThroughput === undefined
    ) {
    ...
    return {
      ...
      AttributeDefinitions:
        (_c = endState.attributeDefinitions) === null || _c === void 0
          ? void 0
          : _c
              .filter((def) =>
                attributeNamesToInclude.includes(def.attributeName),
              )
              .map((def) => usePascalCaseForObjectKeys(def)),
      GlobalSecondaryIndexUpdates: [
        {
  ...
  const gsiRequiresUpdatePredicate = (endStateGSI) => {
    ...
    if (
      ...
      const currentStateGSI = currentStateGSIs.find(
        (gsi) => gsi.IndexName === endStateGSI.indexName,
      );
      if (currentStateGSI) {
        if (
          ((_a = currentStateGSI.ProvisionedThroughput) === null ||
          _a === void 0
            ? void 0
            : _a.ReadCapacityUnits) !==
            ((_b = endStateGSI.provisionedThroughput) === null || _b === void 0
              ? void 0
              : _b.readCapacityUnits) ||
          ((_c = currentStateGSI.ProvisionedThroughput) === null ||
          _c === void 0
            ? void 0
            : _c.WriteCapacityUnits) !==
            ((_d = endStateGSI.provisionedThroughput) === null || _d === void 0
              ? void 0
              : _d.writeCapacityUnits)
        ) {
          return true;
        }
      }
      ...
      GlobalSecondaryIndexUpdates: [
        {
          Update: {
            ...
            ProvisionedThroughput: {
              ReadCapacityUnits:
                (_d = gsiToUpdate.provisionedThroughput) === null ||
                _d === void 0
                  ? void 0
                  : _d.readCapacityUnits,
              WriteCapacityUnits:
                (_e = gsiToUpdate.provisionedThroughput) === null ||
                _e === void 0
                  ? void 0
                  : _e.writeCapacityUnits,
            },
          },
        },
      ],
...
const getStreamUpdate = async (currentState, endState) => {
  ...
  let streamUpdate;
  if (
    ((_a = endState.streamSpecification) === null || _a === void 0
      ? void 0
      : _a.streamViewType) !== undefined &&
    (currentState.StreamSpecification === undefined ||
      currentState.StreamSpecification.StreamEnabled === false)
  ) {
    streamUpdate = {
      StreamEnabled: true,
      StreamViewType: endState.streamSpecification.streamViewType,
    };
  } else if (
    ((_b = endState.streamSpecification) === null || _b === void 0
      ? void 0
      : _b.streamViewType) === undefined &&
    ((_c = currentState.StreamSpecification) === null || _c === void 0
      ? void 0
      : _c.StreamEnabled) === true
  ) {
    streamUpdate = { StreamEnabled: false };
  } else if (
    ((_d = currentState.StreamSpecification) === null || _d === void 0
      ? void 0
      : _d.StreamEnabled) === true &&
    ((_e = endState.streamSpecification) === null || _e === void 0
      ? void 0
      : _e.streamViewType) !== undefined &&
    currentState.StreamSpecification.StreamViewType !==
      ((_f = endState.streamSpecification) === null || _f === void 0
        ? void 0
        : _f.streamViewType)
  ) {
    console.log(
      "Detect stream view type is changed. Disabling stream before the type change.",
    );
...
const getSseUpdate = (currentState, endState) => {
  ...
  if (currentState.SSEDescription) {
    if (
      !((_a = endState.sseSpecification) === null || _a === void 0
        ? void 0
        : _a.sseEnabled)
    ) {
      sseUpdate = {
      ...
      ((_b = endState.sseSpecification) === null || _b === void 0
        ? void 0
        : _b.sseEnabled) === true &&
    ...
    if (
      (_c = endState.sseSpecification) === null || _c === void 0
        ? void 0
        : _c.sseEnabled
    ) {
      sseUpdate = {
...
const requiresTagsUpdate = (currentTags, newTags) => {
  ...
  for (const newTag of newTags) {
    if (!currentTags.find((currentTag) => currentTag.Key === newTag.Key)) {
      ...
      if (
        (currentTag === null || currentTag === void 0
          ? void 0
...
const getDeletionProtectionUpdate = (currentState, endState) => {
  if (
    endState.deletionProtectionEnabled !== undefined &&
    currentState.DeletionProtectionEnabled !==
      endState.deletionProtectionEnabled
  ) {
    return {
      TableName: currentState.TableName,
    ...
    endState.deletionProtectionEnabled === undefined &&
    currentState.DeletionProtectionEnabled === true
  ) {
    return {
      TableName: currentState.TableName,
...
const getTtlUpdate = (currentTTL, endState) => {
  ...
  if (currentTTL && currentTTL.TimeToLiveStatus) {
    if (currentTTL.TimeToLiveStatus === "ENABLED" && currentTTL.AttributeName) {
      ...
      currentTTL.TimeToLiveStatus === "DISABLED" &&
      endTTL &&
      endTTL.enabled
    ) {
      return {
...
const getPointInTimeRecoveryUpdate = (currentPointInTime, endState) => {
  ...
  const currentStatus =
    (_a = currentPointInTime.PointInTimeRecoveryDescription) === null ||
    _a === void 0
      ? void 0
      : _a.PointInTimeRecoveryStatus;
  const endStatus =
    (_b = endState.pointInTimeRecoverySpecification) === null || _b === void 0
      ? void 0
      : _b.pointInTimeRecoveryEnabled;
  ...
  for (const key in obj) {
    if (obj.hasOwnProperty(key) && key !== "") {
      ...
      if (Array.isArray(value)) {
        result[capitalizedKey] = value.map((v) =>
          usePascalCaseForObjectKeys(v),
        );
...
const convertStringToBooleanOrNumber = (obj) => {
  ...
  const fieldsToBeConvertedToNumber = [
    "readCapacityUnits",
    "writeCapacityUnits",
  ];
  for (const key in obj) {
    if (Array.isArray(obj[key])) {
      ...
      if (
        ...
        fieldsToBeConvertedToBoolean.includes(key)
      ) {
        obj[key] = obj[key] === "true";
      } else if (
        !isNaN(Number(obj[key])) &&
        fieldsToBeConvertedToNumber.includes(key)
      ) {
...
const toCreateTableInput = (props) => {
  const createTableInput = {
    ...
    SSESpecification: props.sseSpecification
...
const createNewTable = async (input) => {
  ...
  return {
    ...
    streamArn:
      (_a = result.TableDescription) === null || _a === void 0
        ? void 0
        : _a.LatestStreamArn,
  };
};
...
const isTableReady = async (tableName) => {
  ...
  if (
    ((_a = result.Table) === null || _a === void 0
      ? void 0
      : _a.TableStatus) !== "ACTIVE"
  ) {
    console.log("Table not active yet");
  ...
  if (
    (_b = result.Table.GlobalSecondaryIndexes) === null || _b === void 0
      ? void 0
      : _b.some((gsi) => gsi.IndexStatus !== "ACTIVE" || gsi.Backfilling)
  ) {
    console.log("Some GSI is not active yet");
...
const isKeySchemaModified = (currentSchema, endSchema) => {
  ...
  if (
    (currentHashKey === null || currentHashKey === void 0
      ? void 0
      : currentHashKey.AttributeName) !==
    (endHashKey === null || endHashKey === void 0
      ? void 0
      : endHashKey.attributeName)
  )
    return true;
  ...
  if (
    ...
    return true;
  if (
    (currentSortKey === null || currentSortKey === void 0
      ? void 0
      : currentSortKey.AttributeName) !==
    (endSortKey === null || endSortKey === void 0
      ? void 0
      : endSortKey.attributeName)
  )
    return true;
  return false;
};
const getTypeModifiedAttributes = (currentSchema, endSchema) => {
  ...
  for (const attribute of currentSchema) {
    const endAttribute = endSchema.find(
      (endAttr) => endAttr.attributeName === attribute.AttributeName,
    );
...
const getIndexesContainingAttributes = (currentSchema, attributes) => {
  ...
  const result = currentSchema
    ...
    .filter((index) => {
      var _a;
      return (_a = index.KeySchema) === null || _a === void 0
        ? void 0
        : _a.some((key) => attributes.includes(key.AttributeName));
    })
...
const isTtlModified = (oldTtl, endTtl) => {
  ...
  return (
    oldTtl.enabled !== endTtl.enabled ||
    oldTtl.attributeName !== endTtl.attributeName
  );
};
...
const retry = async (func, successPredicate, settings, failurePredicate) => {
  ...
  do {
    try {
      ...
      console.warn(
        `Retry-able function execution did not match success predicate. Result was [${JSON.stringify(result)}]. Retrying...`,
      );
    } catch (err) {
      console.warn(
        `Retry-able function execution failed with [${err.message || err}]`,
      );
      if (stopOnError) {
    ...
    count++;
    const sleepTime = exponentialBackoff
      ? delayMS * Math.pow(2, count - 1)
      : delayMS;
