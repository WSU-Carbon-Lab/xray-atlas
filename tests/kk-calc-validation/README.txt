...
Bun validation (`bun run test:kk-calc-validation`) uses the same CSV for discrete mirror, makima,
TS-vs-kkcalc optical similarity, and **CSV delta column vs kkcalc-delta-optical-beta**.

kkcalc2 optical pipeline (formula + mass density in g/cm³):
...
CLI reference JSON:
  ...
  uv run python run_reference.py discrete-mirror --csv <path>
