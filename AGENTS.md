...
- Spectrum and NEXAFS plot toolrails: use HeroUI `Toolbar` with `isAttached` pill groups; keep each unrelated cluster in its own `ButtonGroup` or `ToggleButtonGroup` with separators (e.g. home and download separate from inspect, zoom, and pan; delta separate from OD, mu, and beta)—do not merge those into one undivided group or one outer border around all controls.
