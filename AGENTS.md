...
- Prefer app toast patterns over `alert()` for user-visible errors in interactive flows (profile, mutations, and similar); on dense plot pages (for example NEXAFS contribute), keep toast surfaces on semantic token classes and stack them above in-plot overlays using the shared toast z-index layer.
...
- For substantive UI changes, HeroUI upgrades, or broad component migrations, use browser verification when available, the HeroUI migration MCP and guide during planning, wait for content to settle on slow pages before screenshots, and subagents for parallel work when that reduces context load. For avatar or attribution-row layout fixes, verify in the browser that header, contribute **Researchers**, and browse surfaces show the same per-user identity before claiming success.
- Keep header chrome controls (Sign In, theme toggle, external links) at the same visual height: `DefaultButton` should follow HeroUI `size` (`md` for ~40px header rows) rather than hardcoding `h-8` when neighbors use `h-10`.
- Sign-in surfaces should state ORCID-first account creation with a linked https://orcid.org reference and note that GitHub works after profile linking; avoid redundant footer or info boxes on modal and full-page sign-in.
...
- For the account profile dropdown, separate developer-oriented items from standard user actions with a visual divider; group profile, dashboard, **Attribution teams** (`/account/teams`), and **Create team** together, and place admin-only entries such as **User administration** in their own section. On `/users/[orcid]`, limit harsh warnings to the danger zone (trash and transfer only, not a nested accordion); use browse-aligned compact molecule cards with paginated molecules and NEXAFS datasets; show overview totals plus per-year bar charts in the header beside identity (GitHub username/link below ORCID); include a disabled API-keys placeholder under Security; match loading skeletons to the painted layout for faster first paint. For admin consoles such as `/admin/users`, prefer a multi-surface layout over a single dense panel; when search is central, show Command+K using HeroUI `Kbd` with `Kbd.Abbr` for the modifier and `Kbd.Content` for the key.
