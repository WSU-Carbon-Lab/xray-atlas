...
- Prisma 7 and ORCID identity: the CLI reads datasource URLs from root `prisma.config.ts` (explicit `.env` then `.env.local` loading); multi-file schema under `prisma/schema/`; runtime client from `~/prisma/client` with `@prisma/adapter-pg` and pooled `DATABASE_URL`. After schema changes, run `bunx prisma generate` and restart the dev server. When migrations fail on Supabase, follow `docs/compliance-implementation.md` (direct `DIRECT_URL`, `scripts/supabase/` grant/ownership SQL, pooler fallback via Supabase MCP `apply_migration`); ORCID PK uses `bun scripts/migrate-user-orcid-pk-audit.ts` and migration `20260522120000_user_orcid_primary_key`. `next_auth.user.id` is the bare ORCID iD; public `/users/[orcid]` with legacy UUID redirects via `user_legacy_id_redirect` (`~/lib/user-route.ts`, `orcidUserIdSchema` in `~/lib/orcid.ts`). New sign-in is ORCID-only (`openid`); GitHub is secondary after profile link (`DEV_GITHUB_*` locally); Hugging Face and dev-mock auth removed; passkeys documented as known broken. Block `/users/` in `robots.ts` and profile metadata; set `AUTH_URL` to the app origin for OAuth callbacks.
