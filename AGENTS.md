...
- For substantial Prisma migrations, schema changes, and tRPC backend work, delegate to the project `backend-manager` subagent when the user requests it. When the user explicitly asks to apply or fix database changes through the Supabase plugin or Supabase MCP, use that path while keeping Prisma schema and repository migration SQL aligned with what is deployed. When `prisma migrate deploy` hangs on the Supabase pooler, apply DDL via Supabase MCP `apply_migration` (or direct SQL), align `_prisma_migrations`, then run `bunx prisma generate` before relying on new columns in the app.
...
- Prisma 7 in this repo: the CLI reads datasource URLs from root `prisma.config.ts` (explicit `.env` then `.env.local` loading); the schema uses Prisma multi-file layout under `prisma/schema/`, where `schema/schema.prisma` holds only `generator` and `datasource` (no inline URLs) and domain models live in sibling fragment files (for example `nexafs.prisma`, `molecules.prisma`, `publications.prisma`). Runtime `PrismaClient` uses `@prisma/adapter-pg` with pooled `DATABASE_URL` from validated env; import generated client and enums from `~/prisma/client` instead of `@prisma/client`. After schema changes, run `bunx prisma generate` and restart the dev server so selects on new fields do not 500. `prisma migrate deploy` against the pooler can hang—use Supabase MCP `apply_migration` on the direct host, then record the migration in `_prisma_migrations` when the CLI cannot finish. Bulk `bun update --latest` can jump Prisma major and break `postinstall` until pins or a migration land; avoid naive project-wide string replace of `@prisma/client` that also matches `@/…` paths.
