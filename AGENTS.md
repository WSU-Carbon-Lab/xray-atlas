...
- Prefer app toast patterns over `alert()` for user-visible errors in interactive flows (profile, mutations, and similar); on dense plot pages (for example NEXAFS contribute), keep toast surfaces on semantic token classes and stack them above in-plot overlays using the shared toast z-index layer. When Supabase/database is unreachable, surface a degraded-state message on backend-dependent components (home popular molecules, browse, plot viewer)—match existing error-page tone via `~/lib/database-unavailable.ts`, `DatabaseUnavailableState`, and `DatabaseUnavailableProvider`/`useDatabaseUnavailable`; STXM workspace stays usable offline/local-first. On **`SpectrumPlot`**, y-axis zoom/pan must mirror energy-axis behavior (marquee brush, pan, wheel, toolbar +/-)—not Shift-only or gutter-only shortcuts.
...
- Header and sign-in chrome: keep Sign In, theme toggle, and external links at equal height (`DefaultButton` HeroUI `size` `md` on ~40px header rows, not hardcoded `h-8`); sign-in surfaces state ORCID-first account creation with a linked https://orcid.org reference and note GitHub works after profile linking—avoid redundant footer or info boxes on modal and full-page sign-in. **What's New** is not a header-nav sparkle or bell control; unread state belongs on the account avatar dropdown and home-hero `HeroUpdatesRow`—always a vertical stack with Quick start above the typographic blog link at all breakpoints (no md side-by-side row or divider; no sparkle icons).
