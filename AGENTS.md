...
`tests/` is the home for **permanent** automated checks. New tests belong here only when they validate **scientific correctness** against an external physics reference (for example cross-language KK / kkcalc2 regression under `tests/kk-calc-validation/`).

Do **not** add colocated `*.test.ts` under `src/` for Zod mappers, DTO round-trips, authz stubs, UI wiring, or pass-count padding. Prefer typecheck, lint, and manual/browser verification for those. Do not treat green unit-test counts as proof of quality.
...
- `tests/` holds physics-reference validation only; not routine application unit tests.
...
- Align with the project’s **lint** and **format** stack (**oxlint** + type-aware rules via `oxlint-tsgolint`, **oxfmt**); enable or respect **`no-explicit-any`** (or equivalent) so **`any`** cannot land unnoticed; do not disable rules broadly—use the narrowest suppression when unavoidable.
...
- Keep **`oxlint-disable` / `@ts-ignore`** rare and local; prefer code structure that satisfies the rule.
