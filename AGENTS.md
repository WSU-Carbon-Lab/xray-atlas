...
- For the account profile dropdown, separate developer-oriented items from standard user actions with a visual divider; group profile, dashboard, **Attribution teams** (`/account/teams`), **Create team**, and **Pending attributions** (`/account/attributions/pending`, with a count badge on the menu bell when rows await accept/decline) together, and place admin-only entries such as **User administration** in their own section. On `/users/[orcid]`, limit harsh warnings to the danger zone (trash and transfer only, not a nested accordion); use browse-aligned compact molecule cards with paginated molecules and NEXAFS datasets; show overview totals plus per-year bar charts in the header beside identity (GitHub username/link below ORCID); include a disabled API-keys placeholder under Security; match loading skeletons to the painted layout for faster first paint. For admin consoles such as `/admin/users`, prefer a multi-surface layout over a single dense panel; when search is central, show Command+K using HeroUI `Kbd` with `Kbd.Abbr` for the modifier and `Kbd.Content` for the key.
