...
/**
 * Generate a Gaussian peak curve for a given peak.
 * @param peak - Peak data with energy, amplitude, and width
 * @param energyRange - Array of energy values to evaluate the Gaussian at
 * @returns Array of intensity values corresponding to the energy values
 */
export const generateGaussianPeak = (
  ...
  // If width is zero or undefined, return a delta-like function at the peak energy
  if (!width || width <= 0) {
  ...
  // Gaussian formula: A * exp(-0.5 * ((x - μ) / σ)^2)
  // Where A is amplitude, μ is center (energy), and σ is standard deviation (width)
  // For peak fitting, width is often given as FWHM (Full Width at Half Maximum)
  // FWHM = 2.355 * σ, so σ = FWHM / 2.355
