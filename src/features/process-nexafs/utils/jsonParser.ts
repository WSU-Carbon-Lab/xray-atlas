  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (
          jsonData.dataset &&
          Array.isArray(jsonData.dataset) &&
          jsonData.dataset.length > 0
        ) {
          for (const datasetItem of jsonData.dataset) {
            ...
            if (
              !Array.isArray(energyArray) ||
              !Array.isArray(absorptionArray)
            ) {
              continue;
            }
              ...
              if (
                ...
                if (!isNaN(energyNum) && !isNaN(absorptionNum)) {
                  ...
                  if (phi !== undefined && phi !== null) {
                    const phiNum =
                      typeof phi === "number" ? phi : parseFloat(String(phi));
                    if (!isNaN(phiNum)) {
                  ...
                  rawData.push({
                    ...
                    ...(spectrumPoint.theta !== undefined && {
                      theta: spectrumPoint.theta,
                    }),
                    ...(spectrumPoint.phi !== undefined && {
                      phi: spectrumPoint.phi,
                    }),
                  });
                }
              }
          ...
          for (const point of jsonData.data) {
            ...
            const absorption =
              point.absorption ?? point.intensity ?? point.signal;
          ...
          const energyArray = Array.isArray(jsonData.energy)
            ? jsonData.energy
            : [];
          const absorptionArray = Array.isArray(jsonData.absorption)
            ? jsonData.absorption
            : Array.isArray(jsonData.intensity)
              ? jsonData.intensity
              : Array.isArray(jsonData.signal)
                ? jsonData.signal
                : [];
          const thetaArray = Array.isArray(jsonData.theta)
            ? jsonData.theta
            : [];
          ...
          for (let i = 0; i < maxLength; i++) {
            ...
            if (energy !== undefined && absorption !== undefined) {
              ...
              if (thetaArray[i] !== undefined) {
                spectrumPoint.theta =
                  typeof thetaArray[i] === "number"
                    ? thetaArray[i]
                    : parseFloat(String(thetaArray[i]));
              }

              if (phiArray[i] !== undefined) {
                spectrumPoint.phi =
                  typeof phiArray[i] === "number"
                    ? phiArray[i]
                    : parseFloat(String(phiArray[i]));
              }
          ...
          reject(
            new Error("JSON file does not contain valid NEXAFS data structure"),
          );
          return;
        }
