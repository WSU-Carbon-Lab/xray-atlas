...
const tabLinkClass =
  "relative z-0 isolate flex min-h-10 min-w-0 flex-1 items-center justify-center gap-2 rounded-lg px-3 text-sm font-medium transition-colors";
    ...
    <Tabs selectedKey={selectedKey} className="mb-8 w-full min-w-0">
      <Tabs.ListContainer className="w-full min-w-0">
        <Tabs.List
          ...
          className="border-border bg-surface flex min-h-12 w-full min-w-0 flex-wrap gap-0.5 rounded-xl border p-1 *:flex *:min-h-10 *:flex-1 *:items-center *:justify-center *:gap-2 *:rounded-lg *:px-3 *:text-sm *:font-medium *:text-muted *:transition-colors *:[&_svg]:block *:data-[selected=true]:text-accent-foreground sm:flex-nowrap sm:*:min-w-0"
        >
          <Tabs.Tab
            id="molecules"
