...
function DescriptorLegendSwatch({
  swatch,
}: {
 ...
 */
export const PlotDescriptorTraceLegend = memo(
  function PlotDescriptorTraceLegend({
    rows,
    columns,
    channelColumnTitle,
    hiddenTraceIds,
    ...
    const gridTemplateColumns = descriptorTraceLegendGridTemplateColumns(
      columns.length,
    );

    const estimatedBoxHeight = computeDescriptorTraceLegendBoxHeight(
      rows.length,
    );
      ...
      <g ref={setLegendGroupRef} pointerEvents="all">
        ...
        >
          ...
          >
            ...
            >
              <span style={{ textAlign: "left" }}>{channelColumnTitle}</span>
              {columns.map((column) => (
                <span
                  key={column.id}
